...
<select id="tipo" name="tipo" class="browser-default form-control" required="yes">
    ...
    if($_SESSION['tipo_cuenta'] == 3 or $_SESSION['tipo_cuenta'] == 5) echo "disabled"; ?> value="3">Vendedor</option>
    <option <? 
    if($type == 4) echo "selected";
    if($_SESSION['tipo_cuenta'] == 5) echo "disabled"; ?> value="4">Usuario</option>
    <option <? 
    if($type == 5) echo "selected";
    if($_SESSION['tipo_cuenta'] == 5 or $_SESSION['tipo_cuenta'] != 1) echo "disabled"; ?> value="5">Gerencia</option>
</select>
<?php 
}
?>
